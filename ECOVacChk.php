...
$id = $_SESSION['id'];
	...
	$check1 = "SELECT * FROM tblScheduled WHERE courseID = 'ECO101'";
	$result1 = mysqli_query($conn, $check1);
	$num = mysqli_num_rows($result1);
	if($num >= 30){
		$delete2 = "DELETE FROM tblScheduled WHERE id = $id && courseID = 'ECO101'";
		$result2 = mysqli_query($conn, $delete2);
		header("Location: mailECO.php");
	}else{
		$delete3 = "DELETE FROM tblScheduled WHERE id = $id && courseID = 'ECO101'";
